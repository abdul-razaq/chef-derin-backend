/**
 * ORISUN Dining Payment Options Seeder
 * Creates payment methods including Stripe, crypto wallets, and bank transfers
 */
  ...
  async seedPaymentOptions() {
    try {
      ...
      for (const optionData of paymentOptionsData) {
        ...
        const existingOptions = await strapi.entityService.findMany('api::payment-option.payment-option', {
          filters: { name: optionData.name }
        });
        
        if (existingOptions.length === 0) {
          const createdOption = await strapi.entityService.create('api::payment-option.payment-option', {
            data: {
              ...optionData,
      ...
      console.log('🎉 ORISUN payment options seeding completed!');
      
      return { 
        success: true, 
        message: 'All payment options created successfully',
        results: results,
        summary: {
          stripe: results.filter(r => r.type === 'stripe').length,
          cryptocurrency: results.filter(r => r.type === 'cryptocurrency').length,
          bankTransfer: results.filter(r => r.type === 'bank_transfer').length,
